...
### 1. Models — נתונים טהורים (`server/models/`)
...
### 3. RuleEngine — אימות חוקיות (`server/rules/rule_engine.py`)
...
### 4. RealTimeArbiter — תנועות בו-זמניות (`server/arbiter/real_time_arbiter.py`)
...
### 5. GameEngine — מתאם מרכזי (`server/game_engine.py`)
...
| `composition/container.py` | בונה `AppContainer` ומחבר Server/Facade/Components |
...
### 7. GameRunner (`server/game_runner.py`)
